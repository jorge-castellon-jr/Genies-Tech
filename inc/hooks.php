 ...
 * @return array
 */
function thd_body_classes( $classes ) {
...
add_filter( 'body_class', 'thd_body_classes' );

/**
 * Flush out the transients used in thd_categorized_blog.
 */
function thd_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'thd_categories' );
}
add_action( 'delete_category', 'thd_category_transient_flusher' );
add_action( 'save_post',     'thd_category_transient_flusher' );

/**
 * Customize "Read More" string on <!-- more --> with the_content();
 */
function thd_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'thorium-design' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'thd_content_more_link' );
 ...
 */
function thd_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'thorium-design' ) );
}
add_filter( 'excerpt_more', 'thd_excerpt_more' );
 ...
 */
function thd_custom_mime_types( $mimes ) {
	$mimes['svg'] = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'thd_custom_mime_types' );
