 ...
 * @return bool
 */
function thd_categorized_blog() {

	// Get the categories.
	$all_the_cool_cats = get_transient( 'thd_categories' );
	if ( false === $all_the_cool_cats ) {
		...
		set_transient( 'thd_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so thd_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so thd_categorized_blog should return false.
		return false;
	}
}
 ...
 */
function thd_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function thd_get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
	...
	if ( ! is_numeric( $width ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'thorium-design' ) );
	}

	if ( ! is_numeric( $height ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'thorium-design' ) );
	}

	if ( ! ctype_xdigit( $background_color ) ) {
		throw new Exception( esc_html__( 'Please provide a valid hex color value for background_color', 'thorium-design' ) );
	}

	if ( ! ctype_xdigit( $text_color ) ) {
		throw new Exception( esc_html__( 'Please provide a valid hex color value for text_color', 'thorium-design' ) );
	}
	...
	$alt = apply_filters( 'thd_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'thorium-design' ) );
 ...
 * @return string
 */
function thd_get_placeholder_unsplash( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
		'category'			=> '',
